...
class CircularRange extends HTMLElement {
    ...
    #safariVisHandler: (() => void) | undefined;
    #safariPageShowHandler: (() => void) | undefined;
    ...
    connectedCallback() {
        ...
        if (this.shadowRoot) {
            const indices = this.shadowRoot.querySelector('[part="indices"]');
            if (indices) indices.innerHTML = this.#generateIndices();
        }
        this.#renderLabels();
        ...
        if (this.#isSafari) {
            ...
            document.addEventListener('visibilitychange', this.#safariVisHandler as EventListener);
            window.addEventListener('pageshow', this.#safariPageShowHandler as EventListener);
        }
        this.addEventListener('keydown', this.#keydown as EventListener);
        this.addEventListener('pointerdown', this.#pointerdown as EventListener);
    }

    disconnectedCallback() {
        this.removeEventListener('keydown', this.#keydown as EventListener);
        this.removeEventListener('pointerdown', this.#pointerdown as EventListener);
        if (this.#isSafari && this.#safariVisHandler && this.#safariPageShowHandler) {
    ...
    #readAttributes() {
        ...
        this.style.setProperty('--_start', this.#startAngle.toString());
        this.style.setProperty('--_end', this.#endAngle.toString());
    ...
    #convertInitialValue() {
        if (this.#reverse && this.hasAttribute('value')) {
            ...
            this.setAttribute('value', internalValue.toString());
        }
    }
    ...
    #update() {
        ...
        this.setAttribute('aria-valuenow', displayValue.toString());
        ...
        this.style.setProperty('--_value', displayValue.toString());
        this.style.setProperty('--_fill', fillAngle.toString());

        if (this.#reverse) {
            this.style.setProperty('--_fill-start', fillAngle.toString());
            this.style.setProperty('--_fill-range', (this.#endAngle - fillAngle).toString());
        } else {
            this.style.setProperty('--_fill-start', this.#startAngle.toString());
            this.style.setProperty('--_fill-range', (fillAngle - this.#startAngle).toString());
        }
    }
    ...
    #setValue(newValue: number) {
        ...
        this.setAttribute('value', clampedValue.toString());
        this.#internals.setFormValue(clampedValue.toString());
    ...
    #renderLabels() {
        ...
        if (!labels || !this.shadowRoot) return;
